...
function add_value_device(id, status) {
    var newdiv_ul = document.createElement('div');
    newdiv_ul.id = id;
    newdiv_ul.style.marginTop = '50px';
    newdiv_ul.innerHTML = '<ul class="list-group" style="margin-left:30%;text-align:center;width:40%;background-color:black"><li class="list-group-item" style="">Id: ' + id + '</li><li class="list-group-item">Status: ' + status + '</li></ul><button class="btn btn-success" onclick="go_emul(' + id + ')">GO</button>';
...
function add_device(id) {
    $.ajax({
        type: 'GET',
        url: 'http://localhost:10327/api/Devices?idUser=' + id+'&email='+'da',
        async: false,
    }).done(function (data) {
	    ...
	    var st='Wait'
            if(item.Status == 1)
		st='Working'
            add_value_device(item.DeviceId, st);
        });
    }).fail(function (data) {
...
function go_emul(id) {
    ...
    $.ajax({
        type: 'PUT',
        url: 'http://localhost:10327/api/Devices/' + id,
	data: {DeviceId:id, Status:2, EmulationKitId:localStorage.emulationid},
        async: false,
    }).done(function (data) {
